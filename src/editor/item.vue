<template>
  ...
  <div class="variable__value">
    ...
    <or-text-expression v-if="valueTypeLocal !== 'boolean'"
        ...
        :invalid="isInvalidVariableValue"
        placeholder="Value"
        :steps="steps" 
        :step-id="stepId"
        ></or-text-expression>
...
export default {
        ...
        valueTypeLocal: {
          ...
          set(newValue) {
            switch (newValue) {
              case 'string':
              case 'number':
                this.variableValueLocal = (typeof this.variableValue === 'boolean' ? '``' : this.variableValue) || '``';
                break;
              case 'boolean':
              ...
              case 'null':
                ...
                break;
              case 'code':
                this.variableValueLocal = this.variableValue || '``';
                break;
            }
      ...
      methods: {
        validateVariableName (varName, isFromEmit = false) {
          ...
          if (_.isEmpty(varName.trim())) {
            this.errorText = 'The variable is required.';
            ...
            if (duplicates.indexOf(varName) > -1) {
              this.errorText = 'Use another variable name';
              this.isInvalidVariableName = true;
            }
          }
        ...
        validateVariableValue (value, valueType) {
          ...
          if (type === 'number' && !this.$refs.variableValueInput.mergeFieldInUse) {
            const isNumber = /^\d+$/.test(value.slice(1, -1)) || /^\d+\.\d+$/.test(value.slice(1, -1));
            if (!value.replace('``', '').length) {
              this.valueErrorText = 'The value is required.';
      ...
      mounted () {
        ...
        if(this.valueTypeLocal === 'code' && this.$refs.variableValueInput.displayTextInput) {
          //this.$refs.variableValueInput.toggleMode();
          this.$refs.valueTypeSelect.setValue('string');
        }
        ...
        if (this.$parent.$parent.mountedFlag) {
          ...
          this.validateVariableValue (this.variableValue);
        } else {
        ...
        this.$watch(
          () => {
            if (this.$refs.variableValueInput) {
              return this.$refs.variableValueInput.displayTextInput
            }
          }, 
          (newValue) => {
            this.isTextInput = newValue;
            this.valueTypeLocal = newValue ? 'string' : 'code';
          }
        )
      },
    }
